...
def getMyCharacters(connect, user, game_type):
    cur = connect.cursor()
    cur.execute('SELECT id,user_id,game_type,name,level,exp,hp,atk,tactics,project_id FROM CharacterInfo WHERE user_id=%s AND game_type=%s;',(user['id'], game_type, ))
    rows = cur.fetchall()
    characters = []
    cur.close()
    for i in range(len(rows)):
...
def getMyCharacter(connect, user, id):
    cur = connect.cursor()
    cur.execute('SELECT id,user_id,game_type,name,level,exp,hp,atk,tactics,project_id FROM CharacterInfo WHERE user_id=%s AND id=%s;',(user['id'], id, ))
    rows = cur.fetchall()
    cur.close()
    character = {}
    character['id'] = int(rows[0][0])
    character['user_id'] = rows[0][1]
    character['game_type'] = rows[0][2]
    character['name'] = rows[0][3]
    character['level'] = int(rows[0][4])
    character['exp'] = int(rows[0][5])
    character['hp'] = int(rows[0][6])
    character['atk'] = int(rows[0][7])
    character['tactics'] = rows[0][8]
    character['project_id'] = int(rows[0][9])
    return character



game_type_metamodel_id = {'race':1, 'airc':2}
...
def createCharacter(connect, user, name, game_type):
    ...
    id = cur.lastrowid
    cur.close()
    proj = ProjectService.createProject(connect, id, user, name, '', game_type_metamodel_id[game_type]) #use API
    return True
